...
const CertCard: React.FC<{ index: number; cert: TCertification }> = ({ index, cert }) => (
  <motion.article variants={fadeIn("up", "spring", index * 0.2, 0.6)} className="rounded-2xl bg-tertiary p-5 ring-1 ring-black/5 hover:shadow-md">
    ...
    <div className="mt-4">
      {cert.credentialUrl ? (
        <a href={cert.credentialUrl} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-xl border border-white/20 px-3 py-2 text-sm font-medium text-white hover:bg-white/10">
          View Credential
        </a>
      ) : (
...
const Certification: React.FC = () => (
  <>
    ...
    <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2">
      {certifications.map((c, index) => (<CertCard key={`${c.name}-${index}`} index={index} cert={c} />))}
    </div>
  </>
);
