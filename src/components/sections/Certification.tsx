...
const CertCard: React.FC<{ cert: TCertification }> = ({ cert }) => {
  ...
  return (
    ...
    >
      {/* Gambar: aspect tetap 16:9, object-contain, + fallback portrait padding */}
      <div
        className={
          "relative w-full aspect-[16/9] overflow-hidden rounded-xl bg-black/20 " +
          (isPortrait ? "p-4 sm:p-6" : "p-2 sm:p-3")
        }
      >
        <img
...
/** Section Certification + marquee auto-scroll pakai rAF */
const Certification: React.FC = () => {
  ...
  useEffect(() => {
    let rafId = 0;
    let x = 0; // posisi translateX (negatif ke kiri)
    const SPEED = 0.35; // px per frame (naikkan untuk lebih cepat, turunkan untuk lebih lambat)
    ...
    const step = () => {
      ...
      if (track && !paused) {
        ...
        // total width: kita render 2x (data digandakan), jadi reset saat lewat setengah
        const half = track.scrollWidth / 2;
        // Math.abs(x) biar aman saat x negatif
        if (Math.abs(x) >= half) {
          x = 0;
        }
  ...
  return (
    <>
      ...
      {/* Banner auto-scroll: pause on hover/touch */}
      <div
        className="group relative mt-10 w-full overflow-hidden"
